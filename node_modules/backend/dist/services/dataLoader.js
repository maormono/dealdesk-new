...
export class DataLoader {
    ...
    async loadTele2Data() {
        ...
        const jsonData = XLSX.utils.sheet_to_json(sheet, { header: 1 });
        // Data starts at row 2 (index 1)
        const uniqueTadigs = new Set();
        for (let i = 1; i < jsonData.length; i++) {
            const row = jsonData[i];
            if (!row || !row[0])
                continue;
            const tadig = row[0]?.toString().trim();
            if (!tadig || uniqueTadigs.has(tadig))
                continue;
    ...
    searchByTadig(query) {
        ...
        return this.data.filter(item => item.tadig.toUpperCase().includes(searchTerm));
    }
    getAllData() {
        return this.data;
    }
    getComparison(tadig) {
        return this.data.filter(item => item.tadig.toUpperCase() === tadig.toUpperCase());
    }
}
