...
export class A1Parser {
  ...
  async parseFile(buffer: Buffer): Promise<PricingRecord[]> {
    try {
  ...
  private parseRow(row: any[], columnMap: Map<string, number>): PricingRecord | null {
  ...
  private parseTechnologies(value: any): string[] {
    const technologies: string[] = [];
    if (!value) return ['4G']; // Default
    
    const techString = value.toString().toUpperCase();
    if (techString.includes('5G')) technologies.push('5G');
    if (techString.includes('4G') || techString.includes('LTE')) technologies.push('4G');
    if (techString.includes('3G') || techString.includes('UMTS')) technologies.push('3G');
    if (techString.includes('2G') || techString.includes('GSM')) technologies.push('2G');
  ...
  private generateTadigCode(country: string, operator: string): string {
