    ...
    try {
      ...
      `)
        ...
        const results = networks?.map(network => {
            ...
            network.network_restrictions?.forEach((restriction) => {
 ...
 */
router.get('/compare/:tadig', async (req, res) => {
    try {
        ...
        const comparison = {
            tadig,
            sources: {
                A1: data?.find((d) => d.source_name === 'A1'),
                Telefonica: data?.find((d) => d.source_name === 'Telefonica'),
                Tele2: data?.find((d) => d.source_name === 'Tele2')
            }
        };
        res.json({
            success: true,
 ...
 */
router.get('/imsi-fees', async (req, res) => {
    try {
        ...
        res.json({
            ...
            summary: {
                ...
                sources: [...new Set(data?.map((d) => d.source_name) || [])]
            }
        });
    }
    catch (error) {
...
router.get('/by-source', async (req, res) => {
    try {
        ...
        data?.forEach((item) => {
            ...
            bySource[src].count++;
            if (item.imsi_access_fee > 0)
                bySource[src].with_imsi++;
            if (item.is_prohibited)
                bySource[src].prohibited++;
        });
        res.json({
            success: true,
            data,
