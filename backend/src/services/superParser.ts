...
export interface NetworkPricing {
  ...
  // Pricing
  imsiAccessFee: number;  // Monthly IMSI/Access fee in EUR
  dataPerMB: number;      // Data rate per MB
  smsOutgoing?: number;   // SMS MO rate
  smsIncoming?: number;   // SMS MT rate
  voiceOutgoing?: number; // Voice MOC rate
  voiceIncoming?: number; // Voice MTC rate
...
export class SuperParser {
  private allNetworks: Map<string, NetworkPricing> = new Map();
  private restrictionNotes: Map<string, string> = new Map();
   ...
   */
  async parseInvoiceFile(filePath: string): Promise<NetworkPricing[]> {
    console.log('📄 Parsing Invoice file with IMSI fees and notes...');
    
    const workbook = XLSX.readFile(filePath);
    const results: NetworkPricing[] = [];
    ...
    if (workbook.Sheets['Pricelist 2024-11-01']) {
      ...
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (!row[0] || !row[2]) continue; // Skip if no country or TADIG
    ...
    if (workbook.Sheets['Network Access fee per IMSI']) {
      ...
      const imsiData = XLSX.utils.sheet_to_json(imsiSheet, { header: 1 }) as any[][];
   ...
   */
  async parseA1File(filePath: string): Promise<NetworkPricing[]> {
    ...
    for (let i = 8; i < data.length; i++) {
      const row = data[i];
      if (!row[0] || !row[4]) continue; // Skip if no country or TADIG
   ...
   */
  async parseTelefonicaFile(filePath: string): Promise<NetworkPricing[]> {
   ...
   */
  async parseTele2File(filePath: string): Promise<NetworkPricing[]> {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (!tadig || processed.has(tadig)) continue;
      processed.add(tadig);
   ...
   */
  private parseRestrictions(text: string): Partial<NetworkPricing> {
    const lower = text.toLowerCase();
    const result: Partial<NetworkPricing> = {};
   ...
   */
  public getNetworkPricing(tadig: string): NetworkPricing | null {
   ...
   */
  public getRestrictedNetworks(): NetworkPricing[] {
    return Array.from(this.allNetworks.values()).filter(n => 
      n.prohibitedNetwork || n.noRoaming || n.dataNotLaunched
    );
  }
   ...
   */
  public getNetworksWithIMSIFees(): NetworkPricing[] {
   ...
   */
  public comparePricing(tadig: string): {
